...
##### Pixel value
Kα and Kβ net areas being sucessfully calculated, their sum is then attributed to the pixel in the same way as done for the density map. After iterating over the whole batch of spectra, the 2D-matrix is now calculated and must be normalized. This is done in respect to the maximum peak area found within the batch of spectra. **Ps.: This method is still experimental and aims in adding more depth to the image, by displaying the element map in proportion to a maximum value contained in the data.**
