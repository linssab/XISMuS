...
Here you can find information on how the code actually handles the data and how it calculates certain values. It is an overall view, so the user can better understand the underlying processes.
...
## Mapping
This is the "core" of the algorithm. It contains the routines that will calculate and generate the elemental map(s) from the spectra batch. So far it can create a single-element map, multi-element map, density map, ratio map and thickness map. How each one is calculated and generated will be explained in the following paragraphs. Further information regarding the `-usage` is given in _REAMDME.md_ file.
...
In order to generate a true image, the matrix must be transformed into a RGBA format. The first step is to normalize our 2D-matrix of total counts, dividing each element (pixel) for the maximum value contained in the matrix and then multiplying each element by 255.
Nonetheless, since we are creating a density map, a grayscale image is enough for visual interpretation. `ImgMath.colorize()` function will apply a "gray" coloring mask to the 2D-matrix generated by _Mapping.py_ module by associating each element from the 2D-matrix to every color channel, R, G and B. A is maintained at the 255. The elemental maps instead, will be a particular case, where only one color layer is filled. In this way a composite image of elements can be displayed, where each color channel represents an element.
...
### Elemental maps
Differently from the density map, the elemental maps must compute the counts contained in very specific intervals of energy (or channels). Due to the statistical process involved in the acquisition of the data itself, count-to-charge ratio, X-ray soure emission curve, detector efficiency and self-attenuation effect (among others), the peaks observed for each XRF line are represented by a gaussian distribution instead of a discrete line.
The typical XRF line energies, jump ration, fluorescence yields, cross-sections and more for each chemical element are calculated and available in open-source libraries (kudos for the xraylib) as well as in printed XRF handbooks.

The data contained in each spectrum represents the photon counts per channel. Since the calculated energies for the XRF lines are given in KeV, a calibration curve must be calculated to "convert" the channels to energies.
This calibration curve is calculated following the linear regression method. The parameters are extracted from _config.cfg_ file together with other essential configurations to run the algorithm.

The function that creates the elemental map takes a list of elements as an input.
It is up to the algorithm to determine the region of interest (ROI) that represents each of the chemical elements the user wants to be found inside the spectrum data, instead of manually defining it as usual. This is made by reading the energy library _EnergyLib.py_ and accessing the theoretical values of lines Kα and Kβ (or Lα and Lβ if the element has an atomic number higher than 50)¹ for each input element.
To save some computing time, Kβ net peak is only searched for and calculated if a Kα peak for the current element was detected in the spectrum being read (the peak search is an iterative method and searches one element at a time). Or if "ratio" option is set to `True` inside _config.cfg_. If "ratio" is set to `False`, the algorithm will skip all beta lines (Siegbahn definition) regardless  and display elemental maps of the alpha lines only.
...
The algorithm supports two types of ROI set-ups: _simple ROI_ and _auto ROI_.
in short, simple ROI defines the start and end of kα and kβ peaks for the element being currently search through the stacked sum of peaks. The indexes defined by `setROI` function are then applied to each spectrum, "cutting" them in the same range. This can lead to the creation of artefacts or misleading information, but it comes with the advantage of a saving a lot of processing time and of a smoother image. For reducing the amount of artifacts, SNIPBG mode is advised.
Auto ROI, on the other hand, sets the ROI locally, i.e. for each individual spectrum. 
The fact that the calibration isn't always perfect (even with a correlation coefficient of 1) and, that there will always be some slight differences between the observed peak position and the theoretical peak position, makes it necessary that a local definition of ROI is set in order to obtain preciser results. It has the advantage of returning more exact netpeak values than those obtained from simple_roi, but if it fails to detect a peak at one given spectrum for whichever reason (usually if the data is too noisy and the element signal is low), the returned netpeak value for that element and that pixel will be 0, creating a "dead pixel" in the final image.
...
Ej is the peak energy/characteristic line, NOISE is the electronic contribution to the peak width, FANO is the fano factor and 3.85 is the energy required to produce an electron-hole pair in silicon. Usign this approach, FANO and NOISE can be optimized for the whole energy distribution.
...
##### Pixel value
Kα and Kβ netpeaks areas being sucessfully calculated, their sum is then attributed to the pixel in the same way as done for the density map (if "ratio" is to `True`, otherwise only Kα netpeak area is attributed). After iterating over the whole batch of spectra, the 2D-matrix is now calculated and must be normalized.
